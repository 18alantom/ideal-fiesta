...
class TestPurchaseInvoice(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for company, seller, items, funds_account, stock_account, posting_date \
                in _test_records:
            cls.doc_handler.create_and_insert_purchase_invoice(
                company, seller, items, funds_account, 
                stock_account, posting_date
            )
    ...
    def sub_test_item_cost(self, item_number, item_entry):
        with self.subTest(item_number=item_number, item_name=item_entry.item):
            ...
            self.assertEqual(cost, item_entry.cost,
                    "Incorrect calculation for item cost.")

    def sub_test_total_cost(self, doc_number, doc):
        with self.subTest(doc_number=doc_number):
            cost = sum([i.cost for i in doc.items])
            self.assertEqual(cost, doc.cost, 
                    "Incorrect calculation for invoice cost")
...
class DocHandler:
    ...
    def create_and_insert_purchase_invoice(self, company, seller, items, 
            funds_account, stock_account, posting_date):
        
        self._handle_purchase_invoice_dependencies(company, seller, items, 
                funds_account, stock_account)

        purchase_invoice_doc = frappe.get_doc(dict(
            doctype="Purchase Invoice",
            company=company,
            seller=seller,
            items=self._get_autonamed_items(items),
    ...
    def _get_item_autoname(self, item_entry):
        record = frappe.db.get_all("Item", 
                filters={"item_name":item_entry["item"]})
        return record[0]["name"]

    def _get_autonamed_items(self, items):
        items = deepcopy(items)
        for item_entry in items:
            item_entry.update({"item":self._get_item_autoname(item_entry)})
        return items

    def _handle_purchase_invoice_dependencies(self, company, seller, items, 
            funds_account, stock_account):
        self._check_if_exists_and_insert("Company",
                dict(name=company, company_name=company))

        self._check_if_exists_and_insert("Seller", 
                dict(name=seller, seller_name=seller))

        for item_entry in items:
            item_name = item_entry["item"]
            self._check_if_exists_and_insert("Item", 
                    dict(item_name=item_name, item_group="TestGroup"))

        self._check_if_exists_and_insert("Account",
                dict(name=get_account_name(funds_account, company), 
                    account_name=funds_account, company=company, account_type="Bank"))

        self._check_if_exists_and_insert("Account",
                dict(name=get_account_name(stock_account, company), 
                    account_name=stock_account, company=company, account_type="Stock"))

    def _check_if_exists_and_insert(self, doctype, doc_dict):
        if not self._check_if_doc_exists(doctype, doc_dict):
            doc_dict.update(dict(doctype=doctype))
            self._create_and_insert_doc(doc_dict)

    def _check_if_doc_exists(self, doctype, doc_dict):
